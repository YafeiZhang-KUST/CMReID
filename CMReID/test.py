...
from tools import visualize_ranked_results
...
parser.add_argument('--model_path', default='save_model0.3_2_61.67/', type=str, help='model save path')
...
if args.method == 'base':
    net = embed_net(n_class, no_local='off', gm_pool='on', arch=args.arch, share_net=args.share_net, cd=args.cd,
                    m3=args.m3, local_feat_dim=args.local_feat_dim, num_strips=args.num_strips)
else:
...
if dataset == 'sysu':
    ...
    model_path = checkpoint_path + 'sysu_c_tri_cd_on_w_tri_2.0_s6_f256_share_net2_base_gm10_k8_p6_lr_0.01_seed_0_fc_best.t'
    ...
    for trial in range(10):
        ...
        if args.re_rank == 'random_walk':
            ...
            if args.cd == 'off': distmat = random_walk(query_feat_fc, gall_feat_fc)
        elif args.re_rank == 'k_reciprocal':
            distmat_pool = k_reciprocal(query_feat_pool, gall_feat_pool)
            if args.cd == 'off': distmat = k_reciprocal(query_feat_fc, gall_feat_fc)
        elif args.re_rank == 'no':
            # compute the similarity
            distmat_pool = -np.matmul(query_feat_pool, np.transpose(gall_feat_pool))
            if args.cd == 'off': distmat = -np.matmul(query_feat_fc, np.transpose(gall_feat_fc))
        # pool5 feature
        cmc_pool, mAP_pool, mINP_pool = eval_sysu(distmat_pool, query_label, gall_label, query_cam, gall_cam)

        # query = [query_img, query_label]
        # gall = [gall_img, gall_label]
        # _datasets = [query, gall]
        # visualize_ranked_results(distmat_pool, _datasets, trial)
    ...
    for trial in range(10):
        ...
        model_path = checkpoint_path + 'regdb_c_tri_cd_on_w_tri_2.0_s6_f256_share_net2_base_gm10_k4_p8_lr_0.1_seed_0_trial_{}_best.t'.format(
            test_trial)
        if os.path.isfile(model_path):
            print('==> loading checkpoint {}'.format(args.resume))
            checkpoint = torch.load(model_path)
        ...
        if args.tvsearch:
            if args.re_rank == 'random_walk':
                ...
                if args.cd == 'off': distmat = random_walk(gall_feat_fc, query_feat_fc)
            elif args.re_rank == 'k_reciprocal':
                distmat_pool = k_reciprocal(gall_feat_pool, query_feat_pool)
                if args.cd == 'off': distmat = k_reciprocal(gall_feat_fc, query_feat_fc)
            elif args.re_rank == 'no':
                # compute the similarity
                distmat_pool = -np.matmul(gall_feat_pool, np.transpose(query_feat_pool))
                if args.cd == 'off': distmat = -np.matmul(gall_feat_fc, np.transpose(query_feat_fc))
            # pool5 feature
